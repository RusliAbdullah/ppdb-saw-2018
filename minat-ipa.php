	...
	<?php 
	$sql="select * from `20-peminatan-ipa`";
	$hasil=mysql_query($sql)or die('Query Fuzzy Sugeno Error:'.mysql_error());
	?>
	<h4>Peminatan IPA<a class="no-print btn btn-primary btn-sm pull-right" href="profile-cetak.php?minat=ipa">Cetak Peminatan IPA</a></h4>
	<hr>
	...
	if($hasil!=null){
		...
		while($row=mysql_fetch_array($hasil)){
			...
			echo "<td>".$row['ncf_ipa']."</td>";
			echo "<td>".$row['scf_ipa']."</td>";
